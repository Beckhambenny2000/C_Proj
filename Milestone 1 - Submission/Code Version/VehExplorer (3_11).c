...
//Post Game
int scanned, movement;                                                          // Create 2 integer for Global variable
long before_energy,before_shield;                          						// Create 2 long variable
bool File_Print;                                                                // Create 1 boolean varaible
FILE *file;

//Post Game dictionary
typedef struct {                                                                // scan counter
    char character;
    int count;
} Scanned_Counter;

typedef struct {                                                                // travel counter
    char character;
    int count;
    int energy;  
    int shield;	 
...
int midRow = ROWS / 2;                                                          // Calculate the middle row 
int midCol = COLS / 2;                                                          // Calculate the middle column 
...
void Vehicle_Movement();                                                        // Run vehicle movement
void Vehicle_Scan(int radius);                                                  // Run vehicle scan and pass int radius
void Post_Game();                                                               // Run Post game
void F_Scanned_Map();                                                           // Run scanned map and transfer to file

void Scanned_Update(Scanned_Counter counters[], int size, char input);          // Update scan counter
void Scanned_Print(const Scanned_Counter counters[], int size, const char *setName);            // Print scan counter
void T_counter(CharCounter2 counters[], int size, char input, int energyUsed, int shieldUsed);  // Update Travel counter
void printT_counter(const CharCounter2 counters[], int size, const char *setName);              // Print Travel counter

//Matrix
void printscanMatrix() ;                                                        // Print scan Matrix
void printtravelMatrix();                                                       // Print travel matrix
...
void Game()
{
	int radius = 1;                                                             // Create 1 integer variable
	long energyToAllocate;                                                      // Create 1 long variable

    for (int i = 0; i < ROWS; i++)												// Set size for scan matrix 
    {
        for (int j = 0; j < COLS; j++) 
        {
            S_matrix[i][j] = ' ';                                               // Put empty space (' ') into the map
        }
    }
    for (int i = 0; i < ROWS; i++) 												// Set size for travel
    {
        for (int j = 0; j < COLS; j++) 
        {
            T_matrix[i][j] = ' ';                                               // Put empty space (' ') into the map
        }
    }
		...
		if (Error == 0)
		{
			switch(user_input_c)
			{
				...
				case 'x':
					...
					printtravelMatrix(T_matrix, midRow, midCol);                //Print scanned matrix
					break;
					
				case 'c':
					printf("Currently Scanned Terrain Layout : \n");
					printscanMatrix(S_matrix, midRow, midCol);                  //Print travel matrix
					break;
...
void Vehicle_Movement()
{
	...
	{
		case 'w':                                                               // Move Up
			scanresult = SCRelay_Signal_Veh_ScanNorth(&My_vehicle);		        // Scan above terrain
			before_energy = My_vehicle.totEnergyUsed;                           // Store before energy before movement
			My_vehicle = SCRelay_Signal_Veh_MoveUpNorth();	                    // Asking header file to move up
			if (scanresult != '?' && scanresult != '#')                         // Check if scan terrain is ? or #
			{
				midRow -= 1;                                                    // If condition met, move matrix up by 1 unit
				T_matrix[midRow][midCol] = scanresult;                          // Store scan terain into matrix
			}
			break;
			
		case 'a':                                                               // Move Left
			scanresult = SCRelay_Signal_Veh_ScanWest(&My_vehicle);		
		...
		case 's':                                                               //Move Down
			scanresult = SCRelay_Signal_Veh_ScanSouth(&My_vehicle);		
			before_energy = My_vehicle.totEnergyUsed;
		...
		case 'd':                                                               //Move Right
			scanresult = SCRelay_Signal_Veh_ScanEast(&My_vehicle);		
			before_energy = My_vehicle.totEnergyUsed;
	...
	if(scanresult == ' ')                                                       // Check if scan result is ' '
    {
        T_matrix[midRow][midCol] = '_';                                         // If condition met indiate '_' as current position
    }
	...
	Info_vehicle_status();                                                      // Print vehicle status
	printtravelMatrix(T_matrix, midRow, midCol);                                // Print travel matrix
}
void Vehicle_Scan(int radius)
{
	...
	if(radius == 1)
	{
		...
		Scanned_Update(S_1_terrain, sizeof(S_1_terrain) / sizeof(S_1_terrain[0]), scanresult);  //Update the counter of the scanresult for 1 radius
	}	
	else if (radius == 2)
	{
		switch (user_input_c)													
		{
			...
			Scanned_Update(S_2_terrain, sizeof(S_2_terrain) / sizeof(S_2_terrain[0]), scanresult);  //Update the counter of the scanresult for 1 radius
		}
		...
		Info_vehicle_status();                                                  // Print out vehicle status
		printscanMatrix(S_matrix, midRow, midCol);                              // Print out scan matrix
...
void Scanned_Update(Scanned_Counter counters[], int size, char input)
{
    ...
    {
        if (counters[i].character == input)                                     // Check if curent position of counter is the scan result
        {
            counters[i].count++;                                                // Increase couner by 1
        }
    }
}
...
void T_counter(CharCounter2 counters[], int size, char input, int energyUsed, int shieldUsed) 
{
    for (int i = 0; i < size; i++) 
    {
        ...
        {
            counters[i].count++;                                                // Increase couner by 1
            counters[i].energy = energyUsed;                                    // Update latest energy used into the counter
            counters[i].shield = shieldUsed;                                    // Update latest shiend used into the counter
        }
    }
}
...
void Scanned_Print(const Scanned_Counter counters[], int size, const char *setName) 
{
    printf("%s \n", setName);
    for (int i = 0; i < size; i++) {
        printf("Type '%c' : %d \n", counters[i].character, counters[i].count);  // Print out all the data inside scan counter
    }
}
...
void F_Scanned_Map()
{
    fprintf(file, "Currently Scanned Terrain Layout : \n");                     // Print out string into the file
    for (int i = 0; i < ROWS; i++)
    {
        for (int j = 0; j < COLS; j++) 
        {
        	char currentChar = S_matrix[i][j];                                  // Take out char in current matrix position
            if (i == midRow && j == midCol) 
            {
                fprintf(file, "_ ");                                            // If matrix 
            }
            else
            {
